...
def _get_selected_edit_bones(context):
    """Return all selected edit bones, including those in non-editable collections."""
    armature = context.object
    if not armature or armature.type != 'ARMATURE':
        return []
    return [b for b in armature.data.edit_bones if b.select]
...
class BONEROLLER_OT_flip_roll(bpy.types.Operator):
    ...
    def poll(cls, context):
        return context.mode == 'EDIT_ARMATURE' and _get_selected_edit_bones(context)

    def execute(self, context):
        for b in _get_selected_edit_bones(context):
            r1 = b.roll + math.pi
            r2 = b.roll - math.pi
